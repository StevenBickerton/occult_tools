...
open (EVENTS, "$efile") or die "Failed to open $efile\n";
...
while (<EVENTS>) { 
  ...
  die "Error reading record." if (! defined($mgcor) or ! defined($mgchi) );
  $mgcor = 1e-9 if   $mgcor==0.0;
  $cor0 = 1e-9 if $cor0==0.0;
  $mgchi = 1e-9 if $mgchi==0.0;
  $chi0 = 1e-9 if $chi0==0.0;
