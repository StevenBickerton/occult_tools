...
#define H_CONST 6.6260755e-27
#define C_CONST 2.99792458e10
#define EXP 2.718281828
#define K_CONST 1.380658e-16
...
FLOAT planck(FLOAT lamb, FLOAT T) {
  return ( 2.0*H_CONST*C_CONST*C_CONST/pow(lamb,5.0) ) / ( pow(EXP, H_CONST*C_CONST/(lamb*K_CONST*T) - 1.0));
}

COORD2D rotate2D(COORD2D P, FLOAT theta) {
  COORD2D Pprime;
  Pprime.x =  P.x * cos(theta) + P.y * sin(theta);
  Pprime.y = -P.x * sin(theta) + P.y * cos(theta);
  return Pprime;
}
...
int main(int argc, char *argv[])
{
  ...
  thisfile  =  argv[0];
  if ( argc < 3 || argc > 5 ) {
    printf("Usage: %s aa T paramFile [dump]\n", thisfile);
    exit(EXIT_FAILURE);
  } else {
    aa = atof(argv[1]);
    T = atof(argv[2]);
...
#define FMT "%f %f %d %f %f %f %d %f %f %f %d %f"
#else
#define FMT "%le %le %d %le %le %le %d %le %le %le %d %le"
#endif
    
    if ( sscanf (paramLine, FMT, &lambLo, &lambHi, &nLambda, &maxX00, &x00Step, &RStarProj, &Nptsrc, &AU, &offset, &aspectRatio, &order, &angle) != 12 ) {
  ...
  WAVELENGTH lambdas[MAX_NLAMBDA];
  FLOAT w_sum = 0;

  for (i=0; i<nLambda; i++) {
    lambdas[i].lamb = lambLo + i*lambdaStep;
    lambdas[i].w = planck(lambdas[i].lamb*100.0, T); // *100 for cm conversion
    w_sum += lambdas[i].w;
  ...
  //fprintf (stdout, "%.3g %.3g %.3g\n", lambdas[i].lamb, lambdas[i].w, w_sum);
  ...
  int Ncompon = nStars;  // nLambda normalized to 1 by weights.
  int ptSrcCount;
  FLOAT xStar, yStar;
  FLOAT lamb;
  ...
  int nMaxX00 = maxX00 / x00Step + 1;
  int m,q;
  FLOAT Ur[2*nMaxX00], Ui[2*nMaxX00], Ihole[2*nMaxX00], Idisk[2*nMaxX00];
  FLOAT I0;
  ...
  for (i=0; i<nLambda; i++) {
	...
	for (q=0; q<2*nMaxX00; q++) {
	  
	  //DB(1.0);
	  P.x = (q-nMaxX00)*x00Step;
	  
	  Pprime = rotate2D(P,angle);
	  x00 = Pprime.x;
	  y00 = Pprime.y;
  ...
  int aaint = (int) aa;
  int AUint = (int) AU;
  snprintf (outfile, MAX_FILENAME, "fresTA-%05d_%05d", aaint, AUint);
  ...
  fprintf (fp, "# %s \taspectRatio_of_KBO_occulter: \t%.2g\n", LASPRAT, aspectRatio);
  fprintf (fp, "# %s \torder_of_approximation: \t%d\n", LORDER, order);
  fprintf (fp, "# %s \tTNO_rotation_angle: \t%.2f\n", LANGLE, angle*180.0/PI);
  
  for (q=0; q<2*nMaxX00; q++) {
    x00 = x00Step*(q-nMaxX00);
    fprintf(fp,"%.0f %.8f %.8f\n",x00,Ihole[q]/Ncompon,Idisk[q]/Ncompon);
  }
  ...
  if ( dump ) {
    ...
    // stars
    snprintf (outfile, MAX_FILENAME, "fresTA-%05d_%05d.stars", aaint, AUint);
    if ( (fp = fopen(outfile, "w")) < 0 ) {
    ...
    // boxes
    snprintf (outfile, MAX_FILENAME, "fresTA-%05d_%05d.boxes", aaint, AUint);
    if ( (fp = fopen(outfile, "w")) < 0 ) {
